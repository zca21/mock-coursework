...
no_diabetes %>% summarize_all(funs(round(sum(is.na(.))*100 / length(.),3)))
dat %>% summarize_all(funs(round(sum(is.na(.))*100 / length(.),3)))

#note that in the original dataset nearly 5% of patients have a missing A1C value thus are excluded from the anaylsis as cannot be used, we would want to investigate these patients further to see why they had a missing value (was it systematic which could introduce bias or was it simply a error?)
```
...
#We consider removing annual income, and adding education and changing the physical activity variable to either sed_bouted or active_min (-0.64 is correlation between them so choose 1 for the moment)
forward_lm$coefficients
...
lm3<-lm(LBXGH~RIDAGEYR+sed_bouted_30min+BMXBMI+Ethnicity+Education,data=no_diabetes)
AIC(lm3)

#from this we think active_min is best variable but the AIC has increased from lm1 very interesting that one of the exercise variables has not much effect but both dramicatally decreases the AIC we now consider adding an interaction between the 2

lm4<-lm(LBXGH~RIDAGEYR+sed_bouted_30min*active_min+BMXBMI+Ethnicity+Education,data=no_diabetes)
AIC(lm4)

summary(lm4)

#no point in interaction (me thinks)



anova(lm2,forward_lm)
...
#Im goign to assume we are happy with lm2
par(mfrow = c(2, 2))
plot(lm2)

#Plot 1: The residual plot shows residuals are centered around 0 and fairly evenly bunched

#Plot 2: The plots do vear of the straight line of the qq plot at the tails, thus we cannot be confident in the normality assumption

#Plot 3: The scale location shows a fairly straight line thus the variablity of the residual points stays constant with the value of the fitted outcome variable, suggesting constant variance in the residual error (or homoscedasticity) [if this was not the case could try logging the response]

##Plot 4: From this plot we can see that row 717 is a highly influential point as is located in bottom right of plot

#further investigating influential points with a Cook's distance plot
plot(lm2,4)
#we see 304,337 and 717 are all highly influential

lm2.diag.metrics%>%top_n(3, wt = .cooksd)

#Testing with removal of these highly influential points (sensitivity analysis)
sdat<-no_diabetes%>%slice(-304,-337,-717)

lms2<-lm(LBXGH~RIDAGEYR+active_min+BMXBMI+Ethnicity+Education,data=sdat)

plot(lms2,4)

#Messing around with stuff from stats2B notes
glance(lm2)
#plotting partial residual plots fro each of the predicators
map(seq.int(1,5),~termplot(lm2, partial.resid=TRUE, terms=.x))
